  ...
  if (!is.element('menu', xmlApply(g.child, xmlAttrs))){
    ...
    if (newFormat){
      ...
      for (menuNum in 1:numMenus){
        ...
        menuOut = menuHelper(name, phone, street, city, state, zip, this.menu, menu.type, notLastCat)
        data.out = rbind(data.out, menuOut)
      }
    } else {
      notLastCat = T
      menu.type = sub('http:.*/(.*)/$', '\\1', document$file)
      data.out = menuHelper(name, phone, street, city, state, zip, menuHead, menu.type, notLastCat)
    }
    if (is.null(data.out)){
    } else {
...
menuHelper = function(name, phone, street, city, state, zip, this.menu, menu.type, notLastCat){
  data.out = NULL
