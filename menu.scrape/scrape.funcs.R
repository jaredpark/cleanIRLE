get.city.rest.urls = function(state, city){
        ...
        if (length(item.name) == 0){
          ...
          if (is.element('price', xmlApply(this.item, xmlAttrs))){
            ...
            if (sub('(.*) - .*', '\\1', item.price.string) == item.price.string){
              ...
              item.price = NA
              item.low = sub('(.*) - (.*)', '\\1', item.price.string)
              item.high = sub('(.*) - (.*)', '\\2', item.price.string)
            }
          ...
          data.out = rbind(data.out, newline) #, item.description))
        }
      }
    }
